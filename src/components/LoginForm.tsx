...
interface Props {
  ...
  onCodeLogin: (code: string) => Promise<void>;
  loading?: boolean;
  error?: string | null;
}
...
export function LoginForm({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setInstitutesError((e as Error).message);
      } finally {
  ...
  function pickInstitute(inst: Institute) {
    setSelected(inst);
    setQuery(`${inst.name}${inst.city ? ` (${inst.city})` : ''}`);
    setShowList(false);
  }

  async function handlePassword(e: FormEvent) {
    e.preventDefault();
    const instituteCode = resolveInstituteCode(selected, query);
    if (!instituteCode) return;
    await onPasswordLogin(instituteCode, username.trim(), password);
  }

  function startOAuth() {
    sessionStorage.setItem('tablaweb_login_pending', '1');
    window.location.href = getAuthorizeUrl('tablaweb');
  }
        ...
        {mode === 'password' ? (
          <form onSubmit={handlePassword} className="space-y-3">
            <div className="relative">
                    ...
                    <li key={inst.code}>
                      ...
                      >
                        ...
                        <div className="text-[11px] text-slate-500">
                          {inst.city}
                          {inst.city ? ' · ' : ''}
                          <span className="font-mono">{inst.code}</span>
                        </div>
                      </button>
                    </li>
                  ))}
              ...
              {(institutesError || (!institutesLoading && institutes.length === 0)) && (
            ...
            <p className="text-[11px] text-slate-400 text-center">
              Ha <code>unauthorized_client</code> hibát kapsz, használd a „KRÉTA
              oldal” fület.
            </p>
          </form>
        ) : (
          <div className="space-y-3">
            <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              A hivatalos KRÉTA belépő oldalra visz. Utána a redirect URL-t /
              kódot illeszd be ide (tartalék mód, ha a jelszavas belépés nem
              engedélyezett).
            </p>
            <button
              type="button"
              onClick={startOAuth}
              disabled={loading}
              className="w-full rounded-xl border border-[var(--accent,#2563eb)] text-[var(--accent,#2563eb)] py-2.5 text-sm font-semibold"
            >
