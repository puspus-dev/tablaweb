...
/**
 * KRÉTA OAuth authorize URL (PKCE) – WebView / új ablakban nyitható.
 */
export function getAuthorizeUrl(state = 'tablaweb'): string {
  const params = new URLSearchParams({
    prompt: 'login',
    nonce: 'wylCrqT4oN6PPgQn2yQB0euKei9nJeZ6_ffJ-VpSKZU',
    response_type: 'code',
...
async function getPolicyHeaders(
  ...
  const nonceRes = await fetch('/idp/nonce', {
    headers: { 'user-agent': USER_AGENT, accept: '*/*' },
  });
  if (!nonceRes.ok) {
    throw new Error(`Nonce lekérés sikertelen: ${nonceRes.status}`);
  }
  const nonce = (await nonceRes.text()).trim();
  if (!nonce) throw new Error('Üres nonce az IDP-től');

  const message =
    instituteCode.toUpperCase() + nonce + username.toUpperCase();
  const key = await hmacSha512Base64(message, POLICY_KEY_STR);

  // Folio header nevek (kis "p" a policy-ben – HTTP case-insensitive)
  return {
...
function idpFormHeaders(extra: Record<string, string> = {}): HeadersInit {
  return {
...
/**
 * PKCE: authorization code → token (Folio / hivatalos mobil flow).
 */
export async function loginWithCode(code: string): Promise<TokenResponse> {
 ...
 * Ha unauthorized_client, használd a PKCE flowot.
 */
export async function login(creds: AuthCredentials): Promise<TokenResponse> {
  const policy = await getPolicyHeaders(creds.instituteCode, creds.username);
...
export async function refreshAccessToken(
  ...
  if (!res.ok) {
    throw new Error(`Token frissítés sikertelen: ${res.status}`);
  }
...
export function instituteFromToken(accessToken: string): string | null {
...
export async function fetchStudent(
  ...
  if (!res.ok) throw new Error(`TanuloAdatlap hiba: ${res.status}`);
  return res.json();
}
...
export async function fetchTimetable(
  ...
  if (!res.ok) {
    throw new Error(`Órarend lekérés hiba: ${res.status}`);
  }
...
export async function fetchSchoolYearEvents(
  ...
  if (!res.ok) {
    throw new Error(`Tanév rendje hiba: ${res.status}`);
  }
...
export async function fetchTimetableWeeks(
  ...
  const fmt = (x: Date) => x.toISOString().slice(0, 10);
  return fetchTimetable(instituteCode, accessToken, fmt(start), fmt(end));
}

export function instituteBase(instituteCode: string): string {
...
export async function fetchGrades(
  ...
  if (!res.ok) throw new Error(`Jegyek lekérés hiba: ${res.status}`);
  const data = await res.json();
  return Array.isArray(data) ? data : [];
}
